...
def get_latent_indices_from_model_config(model):
    """
    Determine modality-aligned current/future state indices based on model configuration.
    ...
    Returns:
        tuple[list[int], list[int]]: (current_state_indices, future_state_indices)
        The returned indices are ordered by modality so that replacing
        `current_state_indices[i] <- future_state_indices[i]` produces a valid
        next-step autoregressive rollout.

    Examples:
        LIBERO old (state_t=9, min_conditional_frames=4):
        ...
        LIBERO new (state_t=10, min_conditional_frames=4):
            Sequence: [blank, curr_proprio, curr_wrist_img, curr_third_person_img, action,
                      trajectory, future_third_person_img, future_wrist, future_proprio, value]
            Returns: ([1, 2, 3], [8, 7, 6])
    ...
    if state_t == 9 and min_conditional_frames == 4:
        # Original LIBERO setup
        return ([1, 2, 3], [5, 6, 7])
    elif state_t == 10 and min_conditional_frames == 4:
        # Trajectory-aware LIBERO setup:
        # current state order is proprio, wrist, primary image
        # future state order in the sequence is primary image, wrist, proprio
        # so we return modality-aligned indices for autoregressive rollouts.
        return ([1, 2, 3], [8, 7, 6])
...
def extract_keypoint_trajectory_from_latent_sequence(
    output_latent: torch.Tensor, trajectory_shape: tuple, trajectory_indices: torch.Tensor
) -> torch.Tensor:
    """
    Extract the predicted 2D keypoint trajectory from the generated latent sequence.

    Args:
        output_latent (torch.Tensor): Full latent sequence with shape (B, C, T, H, W)
        trajectory_shape (tuple): Target shape of the trajectory: (chunk_size, 2)
        trajectory_indices (torch.Tensor): Batch indices specifying which latent frame contains trajectory

    Returns:
        torch.Tensor: Batch of extracted trajectories with shape (B, chunk_size, 2)
    """
    batch_indices = torch.arange(output_latent.shape[0], device=output_latent.device)
    ...
    assert len(trajectory_shape) == 2, "trajectory_shape should have 2 elements: (chunk_size, 2)!"
    ...
    final_trajectory = torch.mean(all_trajectory_chunks, dim=1)
...
def get_action(
    ...
    with torch.inference_mode():
        ...
        use_keypoint_trajectory = cfg.suite == "libero" and getattr(model.config, "state_t", None) == 10
        ...
        # Add blank placeholder for the projected 2D keypoint trajectory. This is only enabled for the
        # trajectory-aware LIBERO model variant and is injected into latent space later.
        if use_keypoint_trajectory:
            image_sequence.append(blank_image_duplicated.copy())
            keypoint_trajectory_latent_idx = current_sequence_idx
            current_sequence_idx += 1
        else:
            keypoint_trajectory_latent_idx = -1

        if use_keypoint_trajectory:
            # Trajectory-aware LIBERO ordering:
            # future primary image -> future wrist -> future proprio
            if cfg.use_third_person_image:
            ...
            # Original ordering used by the pre-trajectory inference helpers.
            if cfg.use_proprio:
                image_sequence.append(blank_image_duplicated.copy())
                future_proprio_latent_idx = current_sequence_idx
                current_sequence_idx += 1
        ...
        data_batch = {
            ...
            "keypoint_trajectory_latent_idx": torch.tensor(
                [keypoint_trajectory_latent_idx] * batch_size, dtype=torch.int64
            ).cuda(),
        ...
        if use_keypoint_trajectory:
            trajectory_indices = torch.full(
                (batch_size,),
                keypoint_trajectory_latent_idx,
                dtype=torch.int64,
                device=generated_latent_with_action.device,
            )
            keypoint_trajectory = (
                extract_keypoint_trajectory_from_latent_sequence(
                    generated_latent_with_action,
                    trajectory_shape=(cfg.chunk_size, 2),
            ...
            keypoint_trajectory = np.clip(keypoint_trajectory, 0.0, 1.0)
        else:
            keypoint_trajectory = None
        ...
        if generate_future_state_and_value_in_parallel:
            ...
            if cfg.suite == "libero":
                INDICES_TO_REPLACE = [
                    ...
                    8,
                    9,
                ] if use_keypoint_trajectory else [
                    0,
                    1,
                    4,
                    5,
                ]  # new LIBERO: 0 blank, 1 curr proprio, 2 curr wrist, 3 curr primary, 4 action, 5 trajectory, 6 future primary, 7 future wrist, 8 future proprio, 9 value
            elif cfg.suite == "robocasa":
                INDICES_TO_REPLACE = [
                    0,
                    1,
        ...
        if batch_size > 1:
            ...
            if keypoint_trajectory is not None:
                trajectory_list = []
                for traj in keypoint_trajectory:
                    trajectory_list.append([traj[i] for i in range(len(traj))])
                keypoint_trajectory = trajectory_list
            if generate_future_state_and_value_in_parallel:
            ...
            if keypoint_trajectory is not None:
                keypoint_trajectory = keypoint_trajectory[0]
                keypoint_trajectory = [keypoint_trajectory[i] for i in range(len(keypoint_trajectory))]
            if generate_future_state_and_value_in_parallel:
        ...
        return_dict = dict(
            actions=actions,
            keypoint_trajectory=keypoint_trajectory,
            trajectory_prediction=keypoint_trajectory,
            ...
            latent_indices=dict(
                ...
                keypoint_trajectory_latent_idx=keypoint_trajectory_latent_idx,
    ...
    with torch.inference_mode():
        ...
        if cfg.suite == "libero":
            INDICES_TO_REPLACE = [
                ...
                8,
                9,
            ] if "keypoint_trajectory_latent_idx" in data_batch and torch.all(
                data_batch["keypoint_trajectory_latent_idx"] != -1
            ) else [
                0,
                1,
                4,
        ...
        # Generate future state prediction conditioned on action (and trajectory, if present)
        data_batch["num_conditional_frames"] = (
            model.config.min_num_conditional_frames
            + (2 if "keypoint_trajectory_latent_idx" in data_batch and torch.all(data_batch["keypoint_trajectory_latent_idx"] != -1) else 1)
        )
        ...
        while not shutdown_event.is_set():
            try:
                ...
                if cfg.search_depth > 1:
                    ...
                    current_state_indices, future_state_indices = get_latent_indices_from_model_config(model)
                    for depth in range(2, cfg.search_depth + 1):
                        for future_state_latent in future_state_return_dict["future_state_samples_list"]:
                            ...
                            rearranged_next_latent_with_future_state[:, :, current_state_indices] = (
                                next_generated_latent_with_future_state[:, :, future_state_indices]
                            )
                            ################################
                            # Predict next action
                            ################################

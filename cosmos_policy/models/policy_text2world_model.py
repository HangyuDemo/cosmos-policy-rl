...
def replace_latent_with_keypoint_trajectory(
    x0: torch.Tensor, keypoint_trajectory: torch.Tensor, trajectory_indices: torch.Tensor
) -> torch.Tensor:
    """
    Replaces the image latent at the specified trajectory index with the projected 2D keypoint trajectory.

    keypoint_trajectory has shape (B, chunk_size, 2), where each point is normalized to [0, 1].
    The flattened trajectory is repeated to fill the latent frame volume, similar to action/proprio injection.
    """
    batch_indices = torch.arange(x0.shape[0], device=x0.device)
    trajectory_latent = x0[batch_indices, :, trajectory_indices, :, :]

    result = torch.zeros_like(trajectory_latent)
    batch_size, latent_channels, latent_h, latent_w = trajectory_latent.shape

    flat_trajectory = keypoint_trajectory.reshape(batch_size, -1)
    num_trajectory_elements = flat_trajectory.shape[1]
    latent_elements = latent_channels * latent_h * latent_w

    assert num_trajectory_elements <= latent_elements, (
        f"Not enough room in the latent tensor for the keypoint trajectory: "
        f"{num_trajectory_elements} trajectory elements > {latent_elements} latent elements!"
    )

    num_repeats = (latent_elements + num_trajectory_elements - 1) // num_trajectory_elements
    repeated_trajectory = flat_trajectory.repeat(1, num_repeats)[:, :latent_elements]
    flat_result = result.reshape(batch_size, -1)
    flat_result[:, :] = repeated_trajectory
    result = flat_result.reshape(batch_size, latent_channels, latent_h, latent_w)

    new_x0 = x0
    new_x0[batch_indices, :, trajectory_indices, :, :] = result
    return new_x0
...
class CosmosPolicyModelConfig(BaseText2WorldModelConfig):
    ...
    # If True, replace dense self-attention with T-causal self-attention while keeping H/W non-causal.
    # Given the current latent ordering (state -> action -> future -> value), this induces the desired
    # hierarchical autoregressive visibility:
    # - action attends to state + earlier action
    # - future attends to state + action + earlier future
    # - value attends to everything before it
    enable_hierarchical_time_causal_self_attention: bool = False
    # Number of dense blocks to keep when replacing self-attention with NeighborhoodAttention.
    # Set to 0 for fully hierarchical autoregressive self-attention across all blocks.
    hierarchical_self_attention_n_dense_blocks: int = 0
    # Sparse-attention parameters used when enable_hierarchical_time_causal_self_attention=True.
    # window_size=-1 expands to full extent for that dimension inside NeighborhoodAttention,
    # so (-1, -1, -1) means full receptive field while is_causal=(True, False, False) makes only T causal.
    hierarchical_self_attention_natten_parameters: dict = attrs.field(
        factory=lambda: {
            "window_size": (-1, -1, -1),
            "stride": (1, 1, 1),
            "dilation": (1, 1, 1),
...
class CosmosPolicyDiffusionModel(BaseDiffusionModel):
    ...
    def _maybe_enable_hierarchical_time_causal_self_attention(self) -> None:
        ...
        self.net = replace_selfattn_op_with_sparse_attn_op(
            self.net,
            n_dense_blocks=self.config.hierarchical_self_attention_n_dense_blocks,
            natten_parameters=self.config.hierarchical_self_attention_natten_parameters,
        )
        if hasattr(self, "net_ema") and self.net_ema is not None:
            self.net_ema = replace_selfattn_op_with_sparse_attn_op(
                self.net_ema,
                n_dense_blocks=self.config.hierarchical_self_attention_n_dense_blocks,
                natten_parameters=self.config.hierarchical_self_attention_natten_parameters,
            )
    ...
    def training_step(
        ...
        output_batch, kendall_loss, _, _ = self.compute_loss_with_epsilon_and_sigma(
            ...
            keypoint_trajectory=data_batch.get("keypoint_trajectory"),
            keypoint_trajectory_indices=data_batch.get("keypoint_trajectory_latent_idx"),
    ...
    def compute_loss_with_epsilon_and_sigma(
        ...
        keypoint_trajectory: Optional[torch.Tensor],
        keypoint_trajectory_indices: Optional[torch.Tensor],
        ...
        Args:
            ...
            keypoint_trajectory: projected 2D keypoint trajectory aligned with the action chunk
            keypoint_trajectory_indices: indices for keypoint trajectory latent frames
        ...
        # Keypoint trajectory
        if keypoint_trajectory is not None and keypoint_trajectory_indices is not None:
            if torch.all(keypoint_trajectory_indices != -1):
                x0_B_C_T_H_W = replace_latent_with_keypoint_trajectory(
                    x0_B_C_T_H_W,
                    keypoint_trajectory.to(x0_B_C_T_H_W.dtype),
                    trajectory_indices=keypoint_trajectory_indices,
                )
        # Proprio
        ...
        if self.config.mask_loss_for_action_future_state_prediction:
            ...
            if torch.any(demo_idx_B):
                ...
                if keypoint_trajectory_indices is not None and torch.all(keypoint_trajectory_indices != -1):
                    mask_B_T[demo_batch_indices, keypoint_trajectory_indices[demo_batch_indices]] = 1
            ...
            if torch.any(world_idx_B):
                ...
                if keypoint_trajectory_indices is not None and torch.all(keypoint_trajectory_indices != -1):
                    mask_B_T[world_batch_indices, keypoint_trajectory_indices[world_batch_indices]] = 1
        ...
        # Get losses for keypoint trajectory prediction
        if keypoint_trajectory_indices is not None and torch.all(keypoint_trajectory_indices != -1):
            trajectory_diff = (
                x0_B_C_T_H_W[batch_indices, :, keypoint_trajectory_indices, :, :]
                - model_pred.x0[batch_indices, :, keypoint_trajectory_indices, :, :]
            )
            trajectory_diff_demo = trajectory_diff[rollout_data_mask == 0]
            trajectory_diff_world_model = trajectory_diff[world_model_sample_mask == 1]

            demo_sample_trajectory_mse_loss = (trajectory_diff_demo**2).mean()
            demo_sample_trajectory_l1_loss = torch.abs(trajectory_diff_demo).mean()
            world_model_sample_trajectory_mse_loss = (trajectory_diff_world_model**2).mean()
            world_model_sample_trajectory_l1_loss = torch.abs(trajectory_diff_world_model).mean()
            all_samples_trajectory_mse_loss = (trajectory_diff**2).mean()
            all_samples_trajectory_l1_loss = torch.abs(trajectory_diff).mean()
        else:
            demo_sample_trajectory_mse_loss = torch.tensor(float("nan"), device=x0_B_C_T_H_W.device)
            demo_sample_trajectory_l1_loss = torch.tensor(float("nan"), device=x0_B_C_T_H_W.device)
            world_model_sample_trajectory_mse_loss = torch.tensor(float("nan"), device=x0_B_C_T_H_W.device)
            world_model_sample_trajectory_l1_loss = torch.tensor(float("nan"), device=x0_B_C_T_H_W.device)
            all_samples_trajectory_mse_loss = torch.tensor(float("nan"), device=x0_B_C_T_H_W.device)
            all_samples_trajectory_l1_loss = torch.tensor(float("nan"), device=x0_B_C_T_H_W.device)
        ...
        output_batch = {
            ...
            "demo_sample_trajectory_mse_loss": demo_sample_trajectory_mse_loss,  # Trajectory loss for policy
            "demo_sample_trajectory_l1_loss": demo_sample_trajectory_l1_loss,  # Trajectory loss for policy
            ...
            "world_model_sample_trajectory_mse_loss": world_model_sample_trajectory_mse_loss,  # Auxiliary trajectory loss
            "world_model_sample_trajectory_l1_loss": world_model_sample_trajectory_l1_loss,  # Auxiliary trajectory loss
            ...
            "all_samples_trajectory_mse_loss": all_samples_trajectory_mse_loss,
            "all_samples_trajectory_l1_loss": all_samples_trajectory_l1_loss,
        }

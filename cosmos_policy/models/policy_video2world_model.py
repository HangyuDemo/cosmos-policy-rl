...
from cosmos_policy.models.policy_text2world_model import (
    ...
    replace_latent_with_keypoint_trajectory,
...
class CosmosPolicyVideo2WorldModel(CosmosPolicyDiffusionModel):
    ...
    def get_data_and_condition(
        ...
        if "rollout_data_mask" in data_batch:
            ...
            # World model: Set the trajectory frame to 1 as well so future predictions can condition on it
            if "keypoint_trajectory_latent_idx" in data_batch and torch.all(
                ...
                condition.condition_video_input_mask_B_C_T_H_W[
                    batch_indices, :, data_batch["keypoint_trajectory_latent_idx"], :, :
                ] = world_model_sample_mask[:, :, 0, :, :]
            ...
            if self.config.mask_current_state_action_for_value_prediction:
                ...
                # Mask out trajectory frame as well because it is derived from the action chunk and should not
                # leak action information into V(s') prediction when this masking mode is enabled.
                if "keypoint_trajectory_latent_idx" in data_batch and torch.all(
                    data_batch["keypoint_trajectory_latent_idx"] != -1
                ):
                    condition.condition_video_input_mask_B_C_T_H_W[
            ...
            if "keypoint_trajectory" in data_batch and "keypoint_trajectory_latent_idx" in data_batch:
                if torch.all(data_batch["keypoint_trajectory_latent_idx"] != -1):
                    condition.gt_frames = replace_latent_with_keypoint_trajectory(
                        condition.gt_frames,
                        data_batch["keypoint_trajectory"],
    ...
    def get_x0_fn_from_batch(
        ...
        if (
            ...
            # Mask out trajectory frame as well so V(s') inference cannot peek at action-derived trajectory.
            if "keypoint_trajectory_latent_idx" in data_batch and torch.all(
                data_batch["keypoint_trajectory_latent_idx"] != -1
            ):
                condition.condition_video_input_mask_B_C_T_H_W[

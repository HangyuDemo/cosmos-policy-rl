        ...
        with (
            ...
            while True:
                ...
                while True:
                    ...
                    dataload_time = time.perf_counter() - dataload_start_time
                    ...
                    train_step_time = time.perf_counter() - train_step_start_time
                    total_iter_time = dataload_time + train_step_time
                    self.callbacks.on_training_step_batch_end(
                        model, data_batch, output_batch, loss, iteration=iteration
                    )
                    batch_size = None
                    if isinstance(data_batch, dict) and "video" in data_batch and hasattr(data_batch["video"], "shape"):
                        batch_size = data_batch["video"].shape[0]
                    timing_msg = (
                        f"[timing] iter={iteration} "
                        f"batch_size={batch_size if batch_size is not None else 'unknown'} "
                        f"data_time={dataload_time:.4f}s "
                        f"step_time={train_step_time:.4f}s "
                        f"total_time={total_iter_time:.4f}s"
                    )
                    # If the gradients are still being accumulated, continue to load the next training batch.
                    if grad_accum_iter != 0:
                        log.info(f"{timing_msg} grad_accum=on")
                        continue
                    # Do the following when an actual optimizer (update) step has been made.
                    iteration += 1
                    log.info(f"{timing_msg} optimizer_step=done next_iter={iteration}")
                    # Save checkpoint.

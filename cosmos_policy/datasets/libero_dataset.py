...
def infer_eef_positions(robot_states: np.ndarray, ee_pos: np.ndarray | None = None, ee_states: np.ndarray | None = None):
    if ee_pos is not None:
        return ee_pos.astype(np.float32)
    if ee_states is not None:
        return ee_states[:, :3].astype(np.float32)
    if robot_states.ndim != 2 or robot_states.shape[1] < 5:
        raise ValueError(f"Unexpected robot_states shape for EEF extraction: {robot_states.shape}")
    if robot_states.shape[1] >= 9:
        return robot_states[:, 2:5].astype(np.float32)
    return robot_states[:, 1:4].astype(np.float32)


def project_world_points(world_points: np.ndarray, world_to_camera: np.ndarray, intrinsics: np.ndarray) -> np.ndarray:
    points_h = np.concatenate([world_points, np.ones((len(world_points), 1), dtype=np.float64)], axis=1)
    cam_points_h = (world_to_camera @ points_h.T).T
    cam_points = cam_points_h[:, :3]

    projected = np.full((len(world_points), 2), np.nan, dtype=np.float64)
    valid = cam_points[:, 2] > 1e-8
    if np.any(valid):
        pix_h = (intrinsics @ cam_points[valid].T).T
        projected[valid, 0] = pix_h[:, 0] / cam_points[valid, 2]
        projected[valid, 1] = pix_h[:, 1] / cam_points[valid, 2]
    return projected


def sanitize_projected_points(points_2d: np.ndarray, image_height: int, image_width: int) -> np.ndarray:
    safe_points = points_2d.copy()
    fallback = np.array([image_width / 2.0, image_height / 2.0], dtype=np.float64)
    last_valid = fallback

    for idx in range(len(safe_points)):
        point = safe_points[idx]
        if np.isnan(point).any() or np.isinf(point).any():
            safe_points[idx] = last_valid
            continue
        point[0] = np.clip(point[0], 0.0, image_width - 1.0)
        point[1] = np.clip(point[1], 0.0, image_height - 1.0)
        safe_points[idx] = point
        last_valid = point

    return safe_points


def normalize_2d_points(points_2d: np.ndarray, image_height: int, image_width: int) -> np.ndarray:
    normalized = points_2d.astype(np.float32).copy()
    normalized[:, 0] = normalized[:, 0] / max(float(image_width - 1), 1.0)
    normalized[:, 1] = normalized[:, 1] / max(float(image_height - 1), 1.0)
    return normalized


def get_keypoint_trajectory_chunk_with_padding(
    ...
    max_idx = len(eef_positions) - 1
    points = []
    for offset in range(chunk_size):
        idx = min(relative_step_idx + offset, max_idx)
        points.append(eef_positions[idx])
...
class LIBERODataset(Dataset):
    def __init__(
        ...
        use_keypoint_trajectory: bool = True,
        ...
        self.use_keypoint_trajectory = use_keypoint_trajectory
        ...
        if self.demonstration_sampling_prob > 0:  # Only load demos if they are used
            for file in tqdm(hdf5_files):
                with h5py.File(file, "r") as f:
                    ...
                    ee_pos = obs_group["ee_pos"][:].astype(np.float32) if "ee_pos" in obs_group else None
                    ee_states = obs_group["ee_states"][:].astype(np.float32) if "ee_states" in obs_group else None
                    eef_positions = infer_eef_positions(proprio, ee_pos=ee_pos, ee_states=ee_states)
                    if self.use_keypoint_trajectory:
                        if "camera_metadata/agentview" not in f:
                            raise KeyError(f"Expected camera metadata in file: {file}")
                        camera_group = f["camera_metadata/agentview"]
                        intrinsics = camera_group["intrinsics"][:].astype(np.float32)
                        world_to_camera = camera_group["world_to_camera"][:].astype(np.float32)
                    else:
                        intrinsics = None
            ...
            if self.treat_success_rollouts_as_demos:
                for ep_idx, ep_meta in self.rollout_episode_metadata.items():
                    ...
                    self.data[self.num_episodes] = dict(
                        ...
                        suite=suite_name,
                        returns=returns,
                        eef_positions=episode_data["eef_positions"],
                        intrinsics=episode_data["intrinsics"],
                        world_to_camera=episode_data["world_to_camera"],
                    )
    ...
    def _load_rollout_episode_data(self, episode_metadata):
        ...
        with h5py.File(file_path, "r") as f:
            ...
            proprio_raw = f["proprio"][:].astype(np.float32)
            proprio = proprio_raw.copy()
            if "ee_pos" in f:
                eef_positions = f["ee_pos"][:].astype(np.float32)
            elif "ee_states" in f:
                eef_positions = f["ee_states"][:, :3].astype(np.float32)
            else:
                eef_positions = infer_eef_positions(proprio_raw)
            if self.use_keypoint_trajectory:
                if "camera_metadata/agentview" not in f:
                    raise KeyError(f"Expected camera metadata in rollout file: {file_path}")
                camera_group = f["camera_metadata/agentview"]
                intrinsics = camera_group["intrinsics"][:].astype(np.float32)
                world_to_camera = camera_group["world_to_camera"][:].astype(np.float32)
            else:
    ...
    def __getitem__(self, idx):
        ...
        # Add blank image for keypoint trajectory
        if self.use_keypoint_trajectory:
            blank_image = np.zeros_like(decompressed_images[relative_step_idx])
            blank_image = duplicate_array(blank_image, total_num_copies=self.num_duplicates_per_image)
            image_list.append(blank_image)
        ...
        # Add future primary image first so it can depend on state + action + trajectory
        if self.use_third_person_images:
        ...
        # Add future wrist image(s) after the future primary image
        if self.use_wrist_images:
        ...
        # Add future proprio last among future-state targets
        if self.use_proprio:
        ...
        if self.use_keypoint_trajectory:
            image_height, image_width = decompressed_images[relative_step_idx].shape[:2]
            keypoint_trajectory = get_keypoint_trajectory_chunk_with_padding(
                eef_positions=episode_data["eef_positions"],
                relative_step_idx=relative_step_idx,
                chunk_size=self.chunk_size,
                world_to_camera=episode_data["world_to_camera"],
        ...
        sample_dict = {
            ...
            * torch.ones(
            ...
            "keypoint_trajectory_latent_idx": keypoint_trajectory_latent_idx if self.use_keypoint_trajectory else -1,
